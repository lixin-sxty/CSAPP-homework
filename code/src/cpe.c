...
static long int get_cnt(long int index, long int samples, long int maxcnt,
                        sample_t smethod, double bias) {
...
*/
double find_cpe_full(elem_fun_t f, long int maxcnt, long int samples,
                     FILE *data_file, sample_t smethod, double bias,
                     long int verbose) {
  long int i;
  long int cnt;
  double cpe;
  ...
  if (data_file && verbose) {
    ...
    fprintf(data_file, "cpe\t%.2f\tovhd\t%.2f\tavgerr\t\%.3f\tmaxerr\t\%.3f\n",
            cpe, overhead, ls_error(cnt_val, cycle_val, samples, LS_AVG),
