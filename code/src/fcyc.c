...
/* When set, will run code to clear cache before each measurement
   Default = 0
*/
void set_fcyc_clear_cache(long int clear) { clear_cache = clear; }
...
/* Set size of cache block
   Default = 32
*/
void set_fcyc_cache_block(long int bytes) { cache_block = bytes; }
...
/* Value of K in K-best
   Default = 3
*/
void set_fcyc_k(long int k) { kbest = k; }
