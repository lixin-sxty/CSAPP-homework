...
static list_fun test_fun[TEST_COUNT] = {
    list_len,  list_sum,  list_sum2,      pipe_sum2,
    rlist_len, rlist_sum, list_len_clear, list_sum_clear};
static char *test_descr[TEST_COUNT] = {
    "Iterative List Length",       "Iterative List Sum",
    "Pipelined List Sum with old", "Pipelined List Sum with new",
    "Recursive List Length",       "Recursive List Sum",
    "List Length with Clear",      "List Sum with Clear"};
...
static void run_test(long i) {
  ...
  long len = ASIZE;
  mylink(len);
  printf("Description: %s, f(%ld) = %ld, ", test_descr[i], len,
         test_fun[i](main_list));
  myindex = i;
...
static list2_fun test2_fun[TEST2_COUNT] = {list_copy, plist_copy, plist_copy2,
                                           stationary_copy};
...
static test_fun2 case_fun[CASE2_COUNT] = {test2a, test2b, test2c};

static char *case2_descr[CASE2_COUNT] = {"list --> list", "list --> list->next",
                                         "list->next --> list"};
...
static void run_test2(long i, long c) {
  ...
  long len = ASIZE;
  mylink(len);
  printf("Description: %s, %s, ", test2_descr[i], case2_descr[c]);
  myindex = i;
