...
double wrap_list_prod(int t) { return list_prod(list_data[t]); }

char array_u1p1S_prod_descr[] =
    "Array.  Unroll 1x.  Parallel 1x, Standard associativity";
...
char array_u5p5S_prod_descr[] =
    "Array.  Unroll 5x.  Parallel 5x, Standard associativity";
...
char array_u5p1S_prod_descr[] =
    "Array.  Unroll 5x.  Parallel 1x, Standard associativity";
double array_u5p1S_prod(double *array, int cnt) {
  int i;
  double val = 1.0;
  for (i = 0; i <= cnt - 5; i += 5) {
    val = val * array[i] * array[i + 1] * array[i + 2] * array[i + 3] *
          array[i + 4];
  }
  for (; i < cnt; i++)
    val *= array[i];
...
char array_u5p1A_prod_descr[] =
    "Array.  Unroll 5x.  Parallel 1x, Modified associativity";
double array_u5p1A_prod(double *array, int cnt) {
  int i;
  double val = 1.0;
  for (i = 0; i <= cnt - 5; i += 5) {
    val = val * ((array[i] * array[i + 1]) *
