...
void tpsum1(long cnt) { psum1(a, p, (long)cnt); }

void tpsum2(long cnt) { psum2(a, p, (long)cnt); }

void tpsum1a(long cnt) { psum1a(a, p, (long)cnt); }
...
void check(elem_fun_t tpsumX, long cnt) {
  ...
  for (i = 0; i < cnt; i++) {
    if (b[i] != c[i]) {
      printf("Ref: i = %ld, b[i-1] = %.0f, a[i] = %.0f, b[i] = %.0f.  Got "
             "b[i-1] = %.0f, b[i] = %.0f\n",
...
static void usage(char *name) {
  printf("Usage: %s [-q] [-u] [-a][-r] [-l CNT] [-n REPS] [-s SAMPLES] [-b "
         "BIAS] [-k SEED]\n",
         name);
  printf("\t-q\tQuiet mode\n");
