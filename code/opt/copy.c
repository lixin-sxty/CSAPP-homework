...
static copy_fun copier[TEST_COUNT] = {
    write_read, array_clear, array_clear_2, array_clear_3, array_clear_4,
    mem_clear,  array_copy,  array_copy_2,  array_copy_4,
};

static char *test_descr[TEST_COUNT] = {
    "Write-read",      "Array clear",     "Array clear  x2",
    "Array clear  x3", "Array clear  x4", "Mem clear",
    "Array copy",      "Array copy  x2",  "Array copy  x4"};

long idx = 0;

#define CASE_COUNT 4

static void test2a(long n) { copier[idx](data, data, n); }

static void test2b(long n) { copier[idx](data, data + 1, n); }
#if CASE_COUNT == 4
static void test2c(long n) { copier[idx](data + 1, data, n); }

static void test2d(long n) { copier[idx](data, data2, n); }
#endif

typedef void (*test_fun)(long);

static test_fun case_fun[CASE_COUNT] = {test2a, test2b
#if CASE_COUNT == 4
                                        ,
                                        test2c, test2d
#endif
...
static char *case_descr[CASE_COUNT] = {"data --> data", "data --> data+1",
#if CASE_COUNT == 4
                                       "data+1 --> data", "data1 --> data2"
#endif
};

static void run_test(long i, long c) {
  double cpe;
  printf("Description: %s, %s, ", test_descr[i], case_descr[c]);
  idx = i;
