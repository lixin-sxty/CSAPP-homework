...
long ad(long a, long b) { return a < b ? b - a : a - b; }
...
int main(int argc, char *argv[]) {
  ...
  printf(
      "Function: absdiff.     Data hi/lo.   Length %lu.  Cycles/element %.2f\n",
      n, t);
  tcmov1 = t;
  t = run(absdiff_asm, hi_lo_a, hi_lo_b, n);
  printf(
      "Function: absdiff_asm. Data hi/lo.   Length %lu.  Cycles/element %.2f\n",
      n, t);
  thilo = t;

  t = run(absdiff, hi_lo_b, hi_lo_a, n);
  printf(
      "Function: absdiff.     Data lo/hi.   Length %lu.  Cycles/element %.2f\n",
      n, t);
  tcmov2 = t;
  t = run(absdiff_asm, hi_lo_b, hi_lo_a, n);
  printf(
      "Function: absdiff_asm. Data lo/hi.   Length %lu.  Cycles/element %.2f\n",
      n, t);
  tlohi = t;

  t = run(absdiff, alt_a, alt_b, n);
  printf(
      "Function: absdiff.     Data alt.     Length %lu.  Cycles/element %.2f\n",
      n, t);
  tcmov3 = t;
  t = run(absdiff_asm, alt_a, alt_b, n);
  printf(
      "Function: absdiff_asm. Data alt.     Length %lu.  Cycles/element %.2f\n",
      n, t);
  tgood = t;

  printf("Avg(%.2f, %.2f) = %.2f.  Compare to %.2f\n", thilo, tlohi,
         (thilo + tlohi) / 2.0, tgood);

  t = run(absdiff, ran_a, ran_b, n);
  printf(
      "Function: absdiff.     Data random.  Length %lu.  Cycles/element %.2f\n",
      n, t);
  tcmov4 = t;
  t = run(absdiff_asm, ran_a, ran_b, n);
  printf(
      "Function: absdiff_asm. Data random.  Length %lu.  Cycles/element %.2f\n",
      n, t);
  tavg = t;
  printf("cmov times: %.2f, %.2f, %.2f, %.2f.  Avg = %.2f\n", tcmov1, tcmov2,
         tcmov3, tcmov4, (tcmov1 + tcmov2 + tcmov3 + tcmov4) / 4.0);
