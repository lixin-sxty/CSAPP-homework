...
double d_max(double x, double y) { return x < y ? y : x; }

float f_max(float x, float y) { return x < y ? y : x; }
...
double simplefun2(double x) { return EXPR2(x); }

double simplefun3(double x) { return EXPR3(x); }
