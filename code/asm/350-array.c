...
int get_value(array4_3_t A, long i, long j) { return A[i][j]; }

row3_t row0, row1, row2, row3;
int *B[4] = {row0, row1, row2, row3};

int get_value2(long *B[4], long i, long j) { return B[i][j]; }
...
int fix_prod_ele_opt(fix_matrix A, fix_matrix B, long i, long k) {
  int *Aptr = &A[i][0];
      /* Points to elements in row i of A    */ // line:asm:fixprodele:c:aptr
  int *Bptr = &B[0][k]; /* Points to elements in column k of B */
  int *Bend = &B[N][k];
      /* Marks stopping point for Bptr       */ // line:asm:fixprodele:c:bend
  int result = 0;
...
int fix_ele(fix_matrix A, int i, int j) { return A[i][j]; }

/* $begin 350-newvarmatrix-c */
void *new_var_matrix(long n) { return malloc(sizeof(int[n][n])); }
...
int pvar_ele(long n, void *p, long i, long j) { return var_ele(n, p, i, j); }
