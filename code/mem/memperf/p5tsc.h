...
extern __inline__ void wrmsr(const uint32 msr, uint32 hi, uint32 lo) {
  __asm __volatile(
 ...
 */
extern __inline__ void rdtsc(uint32 *hi, uint32 *lo) {
  __asm __volatile(".byte 0xf; .byte 0x31; movl %%edx, %0; movl %%eax, %1"
                   : "=g"(*hi), "=g"(*lo)::"eax", "edx");
}

/* Performance Monitor Counters */

extern __inline__ void spmc(uint32 ecx) {
  __asm __volatile("movl %0, %%ecx         # select counter "
                   :
                   : "g"(ecx)
                   : "ecx");
}
 ...
 *  <*hi>, and the lower 32 bits in <*lo>.
 */
extern __inline__ void rdpmc(uint32 *hi, uint32 *lo) {
  __asm __volatile(".byte 0xf; .byte 0x33; movl %%edx, %0; movl %%eax, %1"
                   : "=g"(*hi), "=g"(*lo)::"eax", "edx");
}

/* 64 bit subtract t1-t0 (result 32 bit integer) */
extern __inline__ int subtract64(uint32 hi0, uint32 lo0, uint32 hi1,
                                 uint32 lo1) {
  uint32 hir, lor;
