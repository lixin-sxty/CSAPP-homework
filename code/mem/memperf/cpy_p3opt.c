  ...
  gettime(&hi0, &lo0);
  {
    ...
    for (k = 0; k < it; k++) {
      ...
      for (j = 0; j < l; j++) {
        ...
        m = m + l;
        while (i <= nn - 3) {
          /* that addition doesn't make much sense, but we don't care about the
           * result... */
          __asm__ __volatile__("
                               movq(% 0),
                               % % mm2 paddd % % mm2, % % mm0 movq(% 1),
                               % % mm3 paddd % % mm3, % % mm1 movq(% 2),
                               % % mm4 paddd % % mm4, % % mm0 "
                               :
  ...
  gettime(&hi0, &lo0);
  {
    ...
    for (k = 0; k < it; k++) {
      ...
      for (j = 0; j < l; j++) {
        ...
        m = m + l;
        while (i <= nn - 2) {
          /* we try to store a float zero value. Not sure if it really works, it
          should
          not be assumed that the floating point register value is preserved and
          that gcc is
