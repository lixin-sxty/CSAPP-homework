/* clock.c
 * Retrofitted to use real-time timers and to get clock information from
 * /proc/cpuinfo
 * (C) R. E. Bryant, 2011
 *
 */
...
/* Simulate cycle counters by using nanosecond timers and then converting to
 * clock cycles */
...
double get_counter() {
  ...
  delta_nsecs = 1e9 * (new_time.tv_sec - last_time.tv_sec) +
                (new_time.tv_nsec - last_time.tv_nsec);
  return delta_nsecs * cpu_ghz;
}

double mhz_full(int verbose, int sleeptime) { return mhz(verbose); }

void start_comp_counter() { start_counter(); }
