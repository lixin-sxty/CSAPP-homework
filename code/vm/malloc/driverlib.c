            ...
            char *status_msg) /* Status message returned to caller */
{
  ...
  if ((hp = gethostbyname(hostname)) == NULL) {
    strcpy(status_msg,
           "Error: DNS is unable to resolve Autolab server address");
    close(clientfd);
    return -1;
  }
  ...
  sprintf(buf, "GET /%s/submitr.pl/?userid=%s&lab=%s&result=%s&submit=submit "
               "HTTP/1.0\r\n\r\n",
  ...
  if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
    strcpy(status_msg,
           "Error: Client unable to read first header from Autolab server");
    close(clientfd);
    return -1;
  }
  ...
  while (strcmp(buf, "\r\n")) {
    if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
      strcpy(status_msg,
             "Error: Client unable to read headers from Autolab server");
      close(clientfd);
      return -1;
    }
  }
  ...
  if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
    strcpy(status_msg,
           "Error: Client unable to read status message from Autolab server");
    close(clientfd);
    return -1;
  }
...
int driver_post(char *userid, char *result, int autograded, char *status_msg) {
  ...
  if (userid && strcmp(userid, "")) {
    status = submitr(SERVER_NAME, SERVER_PORT, COURSE_NAME, userid, LAB, result,
                     status_msg);
    return status;
  }
