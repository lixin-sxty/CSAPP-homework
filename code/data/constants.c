...
char *comp_zero(int x) { return COMP_ZERO(x); }

char *comp_tmin(int x) { return COMP_TMIN(x); }
...
int main(int argc, char *argv[]) {
  ...
  if (terse_mode) {
    if (print_header) {
      printf("word size:dialect:decimal:hex:CZ decimal: CZ hex:cz decimal:cz "
             "hex:2x decimal:2x hex\n");
    }
    printf("%d:%s:%lld:%lld:%s:%s:%s:%s:%lld:%lld\n", word_size, dialect,
           tmin_dec_val, tmin_hex_val, CZd, CZh, czd, czh, TWICEd, TWICEh);
  } else {
    printf("Long word size = %d\n", (int)(8 * sizeof(long)));
    printf("Dialect = %s\n", langs[LANG]);
    printf("    COMP_ZERO\n");
    printf("\t%s\t(0x%" PRIx64 ") %s 0\n", STMIN_DEC, (int64_t)TMIN_DEC,
           COMP_ZERO(TMIN_DEC));
    printf("\t%s\t(0x%" PRIx64 ") %s 0\n", STMIN_HEX, (int64_t)TMIN_HEX,
           COMP_ZERO(TMIN_HEX));

    printf("    COMP_TMIN\n");
    printf("\t%s\t(0x%" PRIx64 ") %s %s\n", STMIN_DEC, (int64_t)TMIN_DEC,
           COMP_TMIN(TMIN_DEC), STMIN_DEC);
    printf("\t%s\t(0x%" PRIx64 ") %s %s\n", STMIN_HEX, (int64_t)TMIN_HEX,
           COMP_TMIN(TMIN_HEX), STMIN_DEC);
    ...
    printf("\t2*%s\t(2*0x%" PRIx64 ") = %lld (0x%" PRIx64 ")\n", STMIN_DEC,
           (int64_t)TMIN_DEC, (int64_t)TWICE(TMIN_DEC),
           (int64_t)TWICE(TMIN_DEC));
    printf("\t2*%s\t(2*0x%" PRIx64 ") = %lld (0x%" PRIx64 ")\n", STMIN_HEX,
           (int64_t)TMIN_HEX, (int64_t)TWICE(TMIN_HEX),
           (int64_t)TWICE(TMIN_HEX));
